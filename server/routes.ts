...
import {
  ...
  insertCampaignUpdateSchema,
  insertCategorySchema,
  insertCampaignExtendRequestSchema
} from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/donations", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      // // Cộng thêm số tiền vào trường raised của campaign
      // await storage.increaseCampaignRaised(validatedData.campaignId, validatedData.amount);
  ...
  // ----------- EXTEND REQUEST ROUTES (4 ROUTE) ------------

  // 1. Tổ chức gửi yêu cầu gia hạn campaign (chờ admin duyệt)
  app.post("/api/campaigns/:id/extend-request", authenticateToken, requireRole('organization'), async (req: AuthRequest, res) => {
    try {
      const organization = await storage.getOrganizationByUserId(req.user!.id);
      if (!organization) {
        return res.status(403).json({ message: "Organization not found" });
      ...
      if (!campaign || campaign.organizationId !== organization.id) {
        return res.status(403).json({ message: "Not authorized to extend this campaign" });
      }

      // type: "deadline" hoặc "target", value: số ngày hoặc số tiền, reason: lý do
      const validatedData = insertCampaignExtendRequestSchema.parse({
        ...req.body,
        campaignId,
        status: "pending"
      });

      const request = await storage.createCampaignExtendRequest(validatedData);
      res.status(201).json(request);
    } catch (error) {
      console.error("Create campaign extend request error:", error);
      res.status(400).json({ message: "Failed to create extend request" });
    }
  });

  // 2. Admin lấy danh sách yêu cầu gia hạn
  app.get("/api/admin/campaign-extend-requests", authenticateToken, requireRole('admin'), async (req, res) => {
    try {
      const status = req.query.status as string | undefined;
      const requests = await storage.getCampaignExtendRequests(status ? { status } : {});
      res.json(requests);
    } catch (error) {
      console.error("Get campaign extend requests error:", error);
      res.status(500).json({ message: "Failed to get campaign extend requests" });
    }
  });

  // 3. Admin duyệt yêu cầu gia hạn
  app.put("/api/admin/campaign-extend-requests/:id/approve", authenticateToken, requireRole('admin'), async (req: AuthRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const request = await storage.getCampaignExtendRequest(id);
      if (!request || request.status !== "pending") {
        return res.status(404).json({ message: "Extend request not found or already processed" });
      }

      // Cập nhật campaign theo loại yêu cầu
      if (request.type === "deadline") {
        // Gia hạn ngày: value là số ngày
        await storage.extendCampaignDeadline(request.campaignId, parseInt(request.value));
      } else if (request.type === "target") {
        // Tăng target: value là số tiền mới
        await storage.extendCampaignTarget(request.campaignId, request.value);
      }

      // Cập nhật trạng thái yêu cầu
      await storage.updateCampaignExtendRequestStatus(id, "approved", {
        adminId: req.user!.id,
        reviewedAt: new Date(),
      ...
      await storage.createAdminLog({
        adminId: req.user!.id,
        actionType: 'extend_campaign',
        targetType: 'campaign_extend_request',
        targetId: id,
        reason: req.body.adminNote || 'Extend request approved',
      });

      res.json({ message: "Extend request approved" });
    } catch (error) {
      console.error("Approve extend request error:", error);
      res.status(500).json({ message: "Failed to approve extend request" });
    }
  });

  // 4. Admin từ chối yêu cầu gia hạn
  app.put("/api/admin/campaign-extend-requests/:id/reject", authenticateToken, requireRole('admin'), async (req: AuthRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const request = await storage.getCampaignExtendRequest(id);
      if (!request || request.status !== "pending") {
      ...
      await storage.updateCampaignExtendRequestStatus(id, "rejected", {
        adminId: req.user!.id,
        reviewedAt: new Date(),
        adminNote: req.body.adminNote || ""
      });
      ...
      await storage.createAdminLog({
        ...
        targetType: 'campaign_extend_request',
        targetId: id,
        reason: req.body.adminNote || 'Extend request rejected',
      });

      res.json({ message: "Extend request rejected" });
    } catch (error) {
      console.error("Reject extend request error:", error);
      res.status(500).json({ message: "Failed to reject extend request" });
    }
  });

  // --------------------------------------------------------

  // Admin routes (duyệt tổ chức, chiến dịch, logs)
  app.get("/api/admin/organizations/pending", authenticateToken, requireRole('admin'), async (req, res) => {
    try {
      const organizations = await storage.getPendingOrganizations();
      res.json(organizations);
  ...
  // API cập nhật thông tin user cá nhân
  app.put("/api/users/me", authenticateToken, async (req: AuthRequest, res) => {
    try {
      // Chỉ cho phép cập nhật các trường: firstName, lastName, email, password
      ...
      // Validate dữ liệu (bắt buộc phải có ít nhất 1 trường)
  ...
  // API cập nhật thông tin tổ chức của user hiện tại
  app.put("/api/organizations/user/me", authenticateToken, requireRole('organization'), async (req: AuthRequest, res) => {
    try {
      // Chỉ cho phép cập nhật name, description
