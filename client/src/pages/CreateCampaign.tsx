  ...
  const createCampaignMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Chiến dịch của bạn đã được gửi để quản trị viên duyệt. Bạn sẽ nhận thông báo khi được phê duyệt.",
      });
      queryClient.invalidateQueries({ queryKey: ["/api/campaigns/organization/me"] });
      setLocation('/org-dashboard');
    },
    onError: (error) => {
      toast({
        ...
        description: "Có lỗi xảy ra khi tạo chiến dịch. Vui lòng thử lại.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(targetAmount) || targetAmount <= 0) {
      toast({
        ...
        description: "Vui lòng nhập số tiền mục tiêu hợp lệ lớn hơn 0.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (deadlineDate <= new Date()) {
      toast({
        ...
        description: "Vui lòng chọn hạn kết thúc ở tương lai.",
        variant: "destructive",
      });
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-neutral-600">
          Tạo một chiến dịch gây quỹ để hỗ trợ sứ mệnh và mục tiêu của tổ chức bạn.
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <Input
                ...
                value={formData.title}
                onChange={handleChange}
                placeholder="Nhập tiêu đề hấp dẫn cho chiến dịch"
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                value={formData.description}
                onChange={handleChange}
                placeholder="Mô tả mục tiêu, cách sử dụng quỹ và tác động của chiến dịch. Hãy cụ thể và hấp dẫn để thu hút nhà hảo tâm."
                rows={6}
                className="mt-1"
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <Label htmlFor="target">Số tiền mục tiêu (VNĐ) *</Label>
                <div className="relative mt-1">
                  <DollarSign className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-4 h-4" />
                  <Input
                    id="target"
            ...
            <div>
              <Label htmlFor="imageUrl">Link ảnh chiến dịch (không bắt buộc)</Label>
              <Input
                id="imageUrl"
                name="imageUrl"
                type="url"
              ...
              />
              <p className="text-sm text-gray-500 mt-1">
                Nhập link ảnh đại diện cho chiến dịch. Nếu không nhập, hệ thống sẽ dùng ảnh mặc định.
              </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start">
                <Tag className="w-5 h-5 text-blue-600 mr-3 mt-0.5" />
                <div>
                  <h4 className="font-medium text-blue-900">Quy trình kiểm duyệt</h4>
                  <p className="text-sm text-blue-700 mt-1">
                    Chiến dịch của bạn sẽ được quản trị viên kiểm duyệt trong 2-3 ngày làm việc. Sau khi được duyệt, chiến dịch sẽ hiển thị công khai và bắt đầu nhận quyên góp.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <ul className="text-sm text-green-700 mt-1 space-y-1">
                    <li>• Đặt tiêu đề rõ ràng, hấp dẫn, thể hiện mục đích chiến dịch</li>
                    <li>• Nêu chi tiết cách sử dụng nguồn quỹ</li>
                    <li>• Đặt số tiền mục tiêu và hạn kết thúc thực tế</li>
                    <li>• Chọn danh mục phù hợp để dễ tiếp cận nhà hảo tâm</li>
                  </ul>
                </div>
              </div>
            </div>
