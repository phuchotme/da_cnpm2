...
export interface IStorage {
  ...
  increaseCampaignRaised(id: number, amount: string): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Cập nhật trực tiếp trường raised (không cộng dồn)
  ...
  // Cộng dồn số tiền vào trường raised
  async increaseCampaignRaised(id: number, amount: string): Promise<void> {
    await db.execute(
      sql`UPDATE campaigns SET raised = raised + ${amount} WHERE id = ${id}`
    );
  }
  ...
  async createDonation(insertDonation: InsertDonation): Promise<Donation> {
    ...
    // Cộng dồn số tiền vào trường raised của campaign
    await this.increaseCampaignRaised(insertDonation.campaignId, insertDonation.amount);
